...
description = "Will take the template, apply the name (if given) then add it as an OAS API to the dashboard or gateway"
...
with open(args.templateFile) as APIFile:
    ...
    if args.name:
        ...
        if 'x-tyk-api-gateway' in APIyaml:
            ...
            if args.verbose:
                print(f'[INFO]Creating API with name: {APIyaml["x-tyk-api-gateway"]["info"]["name"]}, listenPath: {APIyaml["x-tyk-api-gateway"]["server"]["listenPath"]["value"]}')
        else:
            print(f"[FATAL]x-tyk-api-gateway not present in {APIFile}")
            sys.exit(1)

resp = tykInstance.createOASAPI(yaml.dump(APIyaml))
